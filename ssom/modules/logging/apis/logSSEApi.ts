...
class LoggingSSEService {
  ...
  private maxReconnectAttempts: number = 10; // 증가
    ...
    try {
      ...
      this.eventSource.addEventListener('error', (event: any) => {
        ...
        if (shouldRetry) {
          ...
          this.handleCooldown(onLogReceived, onConnectionEvent);
        }
      });
  ...
  private handleReconnect(
    ...
    console.log(`🔄 ${Math.floor(delay / 1000)}초 후 재연결 시도 (${this.reconnectAttempts}/${this.maxReconnectAttempts})`);
  ...
  private handleCooldown(
    ...
    console.log(`❄️ 쿨다운 모드 - ${this.cooldownDelay / 1000}초 후 재시도`);
